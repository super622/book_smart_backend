...
exports.updateTimeSheet = async (req, res) => {
    ...
    if (timeSheetFile == "" || !timeSheetFile) {
        ...
        const facility = await Restau_Manager.findOne({ aic: jobDetail.facilityId }, { contactEmail: 1 });
...
exports.getJob = async (req, res) => {
    try {
        ...
        let biddersList = await Promise.all(bidders.map(async (item) => {
            let bidderInfo = await Restau_User.findOne({ aic: item.caregiverId }, { email: 1, phoneNumber: 1 });
            return [
                item.entryDate,
                item.caregiver,
                "",
...
exports.setAwarded = async (req, res) => {
    ...
    const user = await Restau_User.findOne({ aic: nurse.caregiverId }, { email: 1 } );
...
exports.updateJobTSVerify = async (req, res) => {
    ...
    const clinicalInfo = await Restau_User.findOne({ firstName: JobDetails.nurse.split(' ')[0], lastName: JobDetails.nurse.split(' ')[1] });
    ...
    if (status == 1) {
        ...
        const content1 = `<div id=":18t" class="a3s aiL ">
            ...
            <p><strong>Restau_Manager</strong> : ${JobDetails?.location || ''}</p>
...
const MailTransfer = async (name, subject, content) => {
    ...
    try {
        const clinician = await Restau_User.findOne({ firstName, lastName });
        if (clinician) {
        ...
        Job.findOneAndUpdate({ jobId: request.jobId }, { $set: extracted }, { new: false }, async (err, updatedDocument) => {
            if (err) {
                ...
                const content = `<div id=":18t" class="a3s aiL ">
                        <p>
                        <strong> ${updatedDocument.nurse}: You failed in job:${updatedDocument.jobId} beacuse the Restau_Manager don't accept you.<br></strong>
                        </p>
                ...
                const smsContent = `${updatedDocument.nurse}: You failed in job:${updatedDocument.jobId} beacuse the Restau_Manager don't accept you.`
...
async function generateInovices () {  
    ...
    async function pdfGenerate (invoiceData, key) {
        const invoicesForRestau_Manager = [];
        ...
        invoicesForRestau_Manager.push({ facilityId: key, path: invoicePath });
        invoices.push(...invoicesForRestau_Manager);
    }
...
exports.generateInvoice = async (req, res) => {
    try { 
        ...
        if (invoiceGenerate) {
            ...
            return res.status(404).json({message:'Restau_Manager Invoices Not generated. Pleas try again 30 mins later.'});
        }
    } catch (e) {
        console.log(e);
...
exports.sendInvoice = async (req, res) => {
    ...
    const mailOptions = {
        ...
        to: email,
        subject: `Invoice for Restau_Manager ${facilityId}`,
